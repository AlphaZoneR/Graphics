    ...
    glEnableClientState(GL_VERTEX_ARRAY);
        glBindBuffer(GL_ARRAY_BUFFER, _vbo_derivative(order));
            ...
            if (!order)
            {
                ...
                glDrawArrays(render_mode, 0, static_cast<GLsizei>(point_count));
            }
            else
            {
                ...
                glDrawArrays(render_mode, 0, static_cast<GLsizei>(2 * point_count));
            }
...
GLboolean GenericCurve3::UpdateVertexBufferObjects(GLenum usage_flag)
{
    ...
    coordinate = static_cast<GLfloat *>(glMapBuffer(GL_ARRAY_BUFFER, GL_WRITE_ONLY));
...
GLfloat* GenericCurve3::MapDerivatives(GLuint order, GLenum access_mode) const
{
    ...
    return static_cast<GLfloat *>(glMapBuffer(GL_ARRAY_BUFFER, access_mode));
}
